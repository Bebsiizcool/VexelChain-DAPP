...
export const DashboardPage: React.FC = () => {
    ...
    return (
        <div className="h-[calc(100vh-80px)] overflow-hidden flex">
            ...
            {/* LEFT SIDEBAR: Token List (Horizontal style items, Vertical List) */}
                ...
                <div className="p-4 border-b border-white/5 bg-slate-800/30">
                    <div className="relative">
                        <input
                            ...
                            className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 pl-10 text-sm text-white focus:outline-none focus:border-cyan-500"
                        />
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                    {tokenList.map(t => (
                        <button
                            ...
                            className={`w-full flex items-center justify-between p-3 rounded-lg transition-all ${selectedTokenId === t.id ? 'bg-cyan-500/10 border border-cyan-500/30' : 'hover:bg-white/5 border border-transparent'}`}
                        >
            ...
            <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 flex flex-col gap-4">
                    {/* Simple Header for selected token */}
                    <div className="glass-panel p-4 rounded-xl border border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <img src={CONTEXT_TOKENS[selectedTokenId]?.image} className="w-10 h-10 rounded-full shadow-lg shadow-cyan-500/20" />
                            <div>
                                <h1 className="text-2xl font-bold flex items-center gap-2">
                                    {selectedToken.name} <span className="text-slate-500 text-lg font-normal">/ USD</span>
                                </h1>
                                <div className="flex items-center gap-4 text-sm">
                                    <span className="font-mono text-cyan-400 text-lg">${selectedToken.price.toLocaleString()}</span>
